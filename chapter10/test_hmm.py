...
def test_soft_decoding():
    ...
    prob_nodes, prob_edges = hmm.soft_decoding(emissions, model)
    assert prob_nodes == {10: {'A': 0.8167149234719551, 'B': 0.18328507652804493},
    ...
    prob_nodes, prob_edges = hmm.soft_decoding(emissions, model)
    assert prob_nodes == {10: {'A': 0.27251473363412476, 'B': 0.06877055178886632, 'C': 0.6587147145770089},
    ...
    prob_nodes = hmm.soft_decoding(emissions, model)
    assert prob_nodes, prob_edges == {10: {'A': 0.60230736750807, 'B': 0.39769263249192993},
    ...
    # Crooked casino test case
    emissions, model = fetch_hmm(Path('test/testcase42.txt'))
    prob_nodes, prob_edges = hmm.soft_decoding(emissions, model)
    assert prob_nodes == {11: {'F': 0.6094007027025192, 'B': 0.39059929729748083},
                          10: {'F': 0.6856771415538211, 'B': 0.3143228584461788},
                          9: {'F': 0.6923685424215804, 'B': 0.30763145757841964},
                          8: {'F': 0.6331969970384875, 'B': 0.36680300296151236},
                          7: {'F': 0.62685214597358, 'B': 0.37314785402642003},
                          6: {'F': 0.5444074117128204, 'B': 0.4555925882871795},
                          5: {'F': 0.5150976689415157, 'B': 0.4849023310584843},
                          4: {'F': 0.5325114114284429, 'B': 0.4674885885715571},
                          3: {'F': 0.6004578953426175, 'B': 0.3995421046573826},
                          2: {'F': 0.592938794513603, 'B': 0.40706120548639707},
                          1: {'F': 0.6357426178094568, 'B': 0.36425738219054316}}

    assert prob_edges == {
        10: {('F', 'F'): 0.5877232641889896, ('F', 'B'): 0.0979538773648316, ('B', 'F'): 0.02167743851352957,
             ('B', 'B'): 0.2926454199326492},
        9: {('F', 'F'): 0.6430351573350304, ('F', 'B'): 0.049333385086549955, ('B', 'F'): 0.04264198421879084,
            ('B', 'B'): 0.2649894733596288},
        8: {('F', 'F'): 0.608048394324808, ('F', 'B'): 0.02514860271367961, ('B', 'F'): 0.08432014809677241,
            ('B', 'B'): 0.28248285486474},
        7: {('F', 'F'): 0.5818418357456078, ('F', 'B'): 0.04501031022797216, ('B', 'F'): 0.05135516129287977,
            ('B', 'B'): 0.3217926927335402},
        6: {('F', 'F'): 0.5226487788618297, ('F', 'B'): 0.021758632850990705, ('B', 'F'): 0.10420336711175027,
            ('B', 'B'): 0.35138922117542926},
        5: {('F', 'F'): 0.47794698804900404, ('F', 'B'): 0.0371506808925117, ('B', 'F'): 0.06646042366381642,
            ('B', 'B'): 0.41844190739466786},
        4: {('F', 'F'): 0.473188587935253, ('F', 'B'): 0.05932282349318992, ('B', 'F'): 0.0419090810062627,
            ('B', 'B'): 0.4255795075652944},
        3: {('F', 'F'): 0.5073326784956937, ('F', 'B'): 0.09312521684692372, ('B', 'F'): 0.02517873293274924,
            ('B', 'B'): 0.37436337172463335},
        2: {('F', 'F'): 0.5476176005524671, ('F', 'B'): 0.045321193961135944, ('B', 'F'): 0.05284029479015034,
            ('B', 'B'): 0.35422091069624667},
        1: {('F', 'F'): 0.5617314895392028, ('F', 'B'): 0.07401112827025401, ('B', 'F'): 0.031207304974400156,
            ('B', 'B'): 0.333050077216143}}


def test_baum_welch_learning():
    n_iterations, emissions, alphabet, states, transision_matrix, emission_matrix = fetch_viterbi_learning(
        Path('test/testcase43.txt'))
    trans_m, em_m = hmm.baum_welch_learning(emissions,
                                            alphabet,
                                            states,
                                            transition=transision_matrix,
                                            emission=emission_matrix,
                                            n_iters=n_iterations)

    n_iterations, emissions, alphabet, states, transision_matrix, emission_matrix = fetch_viterbi_learning(
        Path('test/testcase44.txt'))
    trans_m, em_m = hmm.baum_welch_learning(emissions,
                                            alphabet,
                                            states,
                                            transition=transision_matrix,
